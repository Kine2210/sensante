...
export async function POST(request: Request) {
  ...
  try {
    ...
    const resultat = await analyserSymptomes(
      { nom: consultation.patient.nom, prenom: consultation.patient.prenom, age, sexe: consultation.patient.sexe, region: consultation.patient.region },
      consultation.symptomes as string[],
      consultation.notes
    );
    const updated = await prisma.consultation.update({
      where: { id: consultationId },
      data: { diagnosticIa: resultat.diagnostic, confiance: resultat.confiance, statut: "termine" },
      include: { patient: true },
    });
